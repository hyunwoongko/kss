...
class SentenceSplitter(SentenceProcessor):
    ...
    def check_split_end(self) -> Tuple[bool, bool]:
        ...
        Notes:
            분할 종료 규칙:
                1. 현재 문자 이후의 문자가 자모('ㅎ','ㅋ','ㅠ' 등)이면 분할을 뒤로 미룬다.
    ...
    def _sf(self) -> bool:
        ...
        Notes:
            ...
            예외:
                ...
                5. 현재 문자 뒤의 공백(SP)과 종결부호(SP)을 제외한 문자가 '만' '론' '데+..'등 이면 분할하지 않는다.
    ...
    def _etn(self) -> bool:
        ...
        Notes:
            ...
            예외:
                1. 현재 문자 뒤의 공백(SP) 및 대부분 부호를 제외한 다음 문자가 쉼표(,)이면 분할하지 않는다. 단 ,,과 같이 연속되어 쓰이면 분할을 허용한다.
                2. 현재 문자 뒤의 바로 뒤 문자가 공백, 한글 자모음, 이모지, 부호 등이 아니면 분할하지 않는다.
    ...
    def _char(self):
        ...
        Notes:
            ...
            허용:
                듯 규칙 1. 현재 문자가 의존명사(NNB)이면서 '듯'이고 공백(SP) 종결부호(SF)를 제외한 다음 문자가 한글 자모이거나 말 줄임표(SE)일 때
                        현재 문자 뒤로 이어지는 모든 자모 문자 뒤의 문자가 동사가 아닌 경우 분할한다.
        """
