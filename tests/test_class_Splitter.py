...
class KssTest(unittest.TestCase):
    def test_single_quotes(self):
        ...
        splitted = splitter.split_sentences(text)
    ...
    def test_from_sooftware(self):
        splitter_m = kss.Splitter(backend="mecab")
        text = open("test_sooftware.txt", "r", encoding="utf-8").read()
        start = time()
        out = splitter_m.split_sentences(text)
        print(out)
        print(time() - start)

        start = time()
        out = splitter_m.split_chunks(text, max_length=256)
        print(out)
        print(time() - start)

        start = time()
        out = splitter_m.split_chunks(text, max_length=256, overlap=True)
        print(out)
        print(time() - start)
    ...
    def test_fightnyy(self):
        ...
        start = time()
        out = splitter.split_sentences(text)
        print(out)
        print(time() - start)
    ...
    def test_chunks(self):
        ...
        out = splitter.split_chunks(data, max_length=24)
        print(out)
        out = splitter.split_chunks(data, max_length=12, overlap=True)
        print(out)

        out = splitter.split_chunks(
            "전하 아니되옵니다. 부디 용서하옵소서. 소인이 큰 죄를 저질렀사옵니다.\n", max_length=24
        )
        print(out)
        out = splitter.split_chunks(
